...
const ENDPOINT_INVENTORY: EndpointSpec[] = [
  ...
  { panel: 'Node & Graph', method: 'GET', path: '/property-nodes/:id/atom-chain', proxiedRoute: '/api/spine/retrieval/property-nodes/:id/atom-chain', status: 'allowed', notes: 'F1b WDLL 3 — property ledger slots via shared fetchPropertyAtomChain (same path as PE/Gate C); /atoms/trace remains Parcel Trace only' },
  { panel: 'Node & Graph', method: 'GET', path: '/property-nodes/:id/boundary-edges', proxiedRoute: '/api/spine/retrieval/property-nodes/:id/boundary-edges', status: 'allowed', notes: 'CC-A U1 WDLL 6 — stranded StoragePort boundary edges over HTTP' },
  { panel: 'Node & Graph', method: 'GET', path: '/nodes/:id', proxiedRoute: '/api/spine/retrieval/nodes/:id', status: 'allowed', notes: 'CC-A U1 WDLL 1+2 — Control-Tower node detail (parcel/road/boundary-edge) with walkable edges' },
  { panel: 'Atom Inspector', method: 'GET', path: '/atoms/:did', proxiedRoute: '/api/spine/retrieval/atoms/:did', status: 'allowed', notes: 'CC-A U2 WDLL 4 — property atom-by-did inspector (fetchAtomByDid); catalog search remains MCP search_atoms' },
  { panel: 'Node & Graph', method: 'GET', path: '/stats/central-tx-node-graph', proxiedRoute: '/api/spine/retrieval/stats/central-tx-node-graph', status: 'allowed', notes: 'FIX 3 — live G1 tally + depth columns; Bearer RETRIEVAL_API_KEY via fetchCentralTxNodeGraphTally' },
  { panel: 'County Manifest', method: 'GET', path: '/api/county-ledger', proxiedRoute: '/api/spine/cortex/api/county-ledger', status: 'allowed', notes: 'SCORED layer — manifestCells grid + railCapabilities + summary.computedAt; re-read on the live-feed interval' },
  { panel: 'County Manifest', method: 'GET', path: '/api/serving-sweep', proxiedRoute: '/api/spine/cortex/api/serving-sweep', status: 'allowed', notes: 'SERVED layer — route does NOT exist yet (probed 2026-08-19: HTTP 200 text/html, 918 bytes, the cortex SPA fallthrough). SS-W7 is building it; the panel renders a named not-served state until then' },
  { panel: 'County Manifest', method: 'GET', path: '/stats/central-tx-node-graph', proxiedRoute: '/api/spine/retrieval/stats/central-tx-node-graph', status: 'allowed', notes: 'WRITTEN layer — same shared fetchCentralTxNodeGraphTally client as Node & Graph, no second read path. SLOW: 6,177 bytes on a good read, HTTP 000 after 240s on a bad one, so it is on-demand only and never on the live-feed interval' },
  { panel: 'Spine Health', method: 'GET', path: '/health/spine', proxiedRoute: '/api/spine/retrieval/health/spine', status: 'allowed', notes: 'COMPLETE-BASTROP B1 WDLL 6–7 — latest spine_health_probe summary; fetchSpineHealthSummary' },
  { panel: 'Spine Health', method: 'GET', path: '/health/spine/run', proxiedRoute: '/api/spine/retrieval/health/spine/run', status: 'allowed', notes: 'COMPLETE-BASTROP B1 — run Bastrop probe pack + persist; runSpineHealthPack' },

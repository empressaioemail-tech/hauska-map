...
> **UPDATED 2026-07-14 (panel-bug pass).** Parcel Trace resolve rewired to live endpoints: POST `/api/plan-review/geocode` (already allowlisted since PR #21-era `cortexPostPaths` — no proxy change needed) + GET `/api/brokerage/v1/place/:placeKey/atoms`; the old GET `/api/brokerage/v1/place/resolve` row (SPA fallthrough, never sent the address) is superseded. Also recorded the MCP `search_atoms` client contract: `entity_type` enum is hyphenated (underscores → -32602), `jurisdiction` matches exact underscored tenant ids only (client normalizes, e.g. "Bastrop, TX" → `bastrop_tx`).
>
...
| Node & Graph (property chain) | GET | `fetchPropertyAtomChain` → retrieval `/property-nodes/:id/atom-chain` | Bearer RETRIEVAL_API_KEY (server-side) | ✅ live (F1b 2026-07-25) — WDLL 3 |
| Node & Graph (county tally) | GET | `fetchCentralTxNodeGraphTally` → retrieval `/stats/central-tx-node-graph` via `/api/spine/retrieval/stats/central-tx-node-graph` | Bearer RETRIEVAL_API_KEY (server-side) | ✅ live (FIX 3 2026-07-26); static artifact is last-resort fallback only |
| County Manifest — SCORED layer | GET | `/api/county-ledger` | service Bearer | ✅ live — 2,121,656 bytes, `summary.computedAt` 2026-08-14T17:41:22.500Z when probed 2026-08-19; re-read on a 60s live-feed interval, and a re-read that does not move `computedAt` is reported as staleness, never as a refresh |
| County Manifest — WRITTEN layer | GET | retrieval `/stats/central-tx-node-graph` via `/api/spine/retrieval/stats/central-tx-node-graph` | Bearer RETRIEVAL_API_KEY (server-side) | ⚠️ live but SLOW and FLAKY — two probes 2026-08-19: HTTP 200 / 6,177 bytes, then HTTP 000 after 240s. Read on demand only, never on mount and never on the feed interval; a failure renders as a named absence carrying the URL and status. Same shared `fetchCentralTxNodeGraphTally` client as Node & Graph — no second read path |
| County Manifest — SERVED layer | GET | `/api/serving-sweep` | service Bearer | 🚫 route does not exist — probed 2026-08-19: HTTP 200 `text/html`, 918 bytes, the documented SPA fallthrough. SS-W7 is building it. Until then the panel renders a named not-served state and accepts a loaded report artifact, labelled ARTIFACT and never live |
| Spine Health (board) | GET | `fetchSpineHealthSummary` → retrieval `/health/spine` via `/api/spine/retrieval/health/spine` | Bearer RETRIEVAL_API_KEY (server-side) | ✅ COMPLETE-BASTROP B1 — latest `spine_health_probe` summary |
| Spine Health (run) | GET | `runSpineHealthPack` → retrieval `/health/spine/run` via `/api/spine/retrieval/health/spine/run` | Bearer RETRIEVAL_API_KEY (server-side) | ✅ Bastrop source+engine pack + persist |
| Parcel Trace (place resolve) | POST | `/api/plan-review/geocode` (address in body → `{placeKey, lat, lng, confidence}`; 422 `{error:"geocode_miss"}` on a miss) | service Bearer (server-side) | ✅ live (2026-07-14) — replaces the nonexistent GET `/api/brokerage/v1/place/resolve` (SPA fallthrough), which also never sent the address |
| Parcel Trace (place atoms) | GET | `/api/brokerage/v1/place/:placeKey/atoms` (`{placeKey, jurisdictionKey, atomCount, atoms}`; 404 `{error:"geocode_miss"}` for unknown place keys) | service Bearer (server-side) | ✅ live (2026-07-14) |

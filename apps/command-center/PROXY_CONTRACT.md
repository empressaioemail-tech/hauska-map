...
| Node & Graph (atom trace) | GET | **same shared client** as Parcel Trace (`fetchAtomTrace` → retrieval `/atoms/trace/:did`) | Bearer RETRIEVAL_API_KEY (server-side) | ✅ live (F1b 2026-07-25) — WDLL 3; no second tracer |
| Node & Graph (county tally) | GET | static Gate A SELECT artifact `/central_tx_node_graph_tally.json` (G1 shape) | n/a (committed live-SELECT snapshot) | ✅ wired (F1b); live re-SELECT remains Phase-0 script |

...
function selfTest() {
  const cases = [
    ...
    {
      name: "WALK: .css files are scanned (pe-mobile.css was a leftover hole)",
      ok: chromeFiles().some((p) => p.split("\\").join("/").endsWith(".css")),
    },
    {
    ...
    // ---- legacy alias literals (P-96 item 5) ----
    {
      name: "ALIAS: a hex on a legacy name is found (NOT VACUOUS)",
      ok:
        legacyAliasLiteralHits(
          "/* LEGACY ALIASES */\n:root {\n  --brand-blue: #86ADDF;\n}\n",
        ).length === 1,
    },
    {
      name: "ALIAS: a px literal on a legacy name is found",
      ok:
        legacyAliasLiteralHits(
          "/* LEGACY ALIASES */\n:root {\n  --btn-pad-y: 10px;\n}\n",
        ).length === 1,
    ...
    {
      name: "ALIAS: var() is allowed",
      ok:
        legacyAliasLiteralHits(
          "/* LEGACY ALIASES */\n:root {\n  --brand-blue: var(--ss-blue);\n}\n",
        ).length === 0,
    },
    {
      name: "ALIAS: color-mix of a token is allowed",
      ok:
        legacyAliasLiteralHits(
          "/* LEGACY ALIASES */\n:root {\n  --atom-accent-bg: color-mix(in oklab, var(--ss-atom) 13%, transparent);\n}\n",
        ).length === 0,
    },
    {
      name: "ALIAS: --ss-* vocabulary is not an alias",
      ok:
        legacyAliasLiteralHits(
          "/* LEGACY ALIASES */\n:root {\n  --ss-blue: #86ADDF;\n}\n",
        ).length === 0,
 ...
 */
/**
 * P-96 item 5. After the LEGACY ALIASES marker, a name that is not --ss-*
 * may only resolve through var() or color-mix(). A hex, rgb(), or px literal
 * is a second source of truth sitting under a header that forbids it.
 */
export function legacyAliasLiteralHits(css) {
  const start = css.indexOf("LEGACY ALIASES");
  if (start < 0) return ["LEGACY ALIASES marker missing"];
  const clean = stripComments(css.slice(start));
  const hits = [];
  const re = /(--(?!ss-)[a-z0-9-]+)\s*:\s*([^;]+);/gi;
  for (const m of clean.matchAll(re)) {
    const val = m[2].trim();
    if (/^var\(/.test(val) || /^color-mix\(/.test(val)) continue;
    hits.push(`${m[1]}: ${val}`);
  }
  return hits;
}
...
/**
 * Recompute colour-literal coverage on this tree. The 7.1% figure was a
 * snapshot of one instrument (hex in .tsx). Printing a live figure every
 * run is the control; baking a new percentage into this file is the same
 * defect one level up.
 *
 * oldSeen  = hex hits in .tsx only (the pre-Stone instrument)
 * measured = all four forms across walked .ts / .tsx / .css, islands and
 *            the token file excluded from the bill (they are allowed)
 */
export function colorCoverage(files = chromeFiles()) {
  let oldSeen = 0;
  let measured = 0;
  const breakdown = { hex: 0, triples: 0, hsl: 0, named: 0 };
  let walked = 0;
  for (const abs of files) {
    const rel = relative(ROOT, abs).split("\\").join("/");
    if (isIsland(rel) || isTokenFile(rel)) continue;
    walked += 1;
    const src = readFileSync(abs, "utf8");
    const b = colorLiteralCounts(src);
    breakdown.hex += b.hex;
    breakdown.triples += b.triples;
    breakdown.hsl += b.hsl;
    breakdown.named += b.named;
    const total = b.hex + b.triples + b.hsl + b.named;
    measured += total;
    if (rel.endsWith(".tsx")) oldSeen += b.hex;
  }
  return { walked, oldSeen, measured, breakdown };
}
...
function main() {
  ...
  const aliasHits = legacyAliasLiteralHits(readRel("src/styles/pe-tokens.css"));
  for (const hit of aliasHits) {
    failures.push(
      `pe-tokens.css legacy alias carries its own literal (${hit}). Point at --ss-* via var() or color-mix().`,
    );
  }

  const cov = colorCoverage();
  const oldPct =
    cov.measured === 0 ? "n/a" : `${((100 * cov.oldSeen) / cov.measured).toFixed(1)}%`;
  console.log(
    `COVERAGE walked ${cov.walked} billed chrome files — ` +
      `hex ${cov.breakdown.hex} triples ${cov.breakdown.triples} ` +
      `hsl ${cov.breakdown.hsl} named ${cov.breakdown.named} ` +
      `measured ${cov.measured}. ` +
      `Old instrument (hex in .tsx only) would see ${cov.oldSeen}/${cov.measured} (${oldPct}). ` +
      `New instrument sees ${cov.measured}/${cov.measured} of the four forms it names.`,
  );

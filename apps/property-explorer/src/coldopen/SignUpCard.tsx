...
//
// The cold-open sign-up card. It floats over a LIVE, dimmed map (the real app
// running behind a CSS scrim — never a screenshot). One headline, three
// bullets, one "Continue with Google" button, and a "Just browse the map"
// escape hatch (browse is anonymous and needs no auth).
//
// SEAM (Track D — auth/OAuth): onContinue is STUBBED. The real Google OAuth
// flow is a parallel track. For now the button is a no-op that dismisses the
// card into anonymous browse, so the skeleton is fully explorable.
...
export function SignUpCard({ onDismiss }: { onDismiss: () => void }) {
  ...
  // TODO(Track D — auth): replace with the real Google OAuth flow. Today this
  // is a no-op that drops the user into anonymous browse.
  const onContinueWithGoogle = () => {
    setBusy(true);
    // no-op stub — parallel track owns the OAuth exchange.
    setTimeout(() => {
      setBusy(false);
      onDismiss();
    }, 150);
